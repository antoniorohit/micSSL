...
def approxTheta(D):
    ...
    while(abs(err) > ERR_THRESH):
        err = D - math.sqrt(K1 - K2*math.cos(theta + 2*math.pi/3)) + math.sqrt(K1 - K2*math.cos(theta))
        theta += 0.1
...
ser = serial.Serial('/dev/tty.usbmodem1412', 921600, timeout=0.1)
